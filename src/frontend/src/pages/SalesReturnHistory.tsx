...
function EditReturnSheet({
  ...
  onClose: () => void;
  onSave: (r: SalesReturnRecord) => void;
}) {
  const { data: medicines = [] } = useGetMedicines();
  const [form, setForm] = useState<SalesReturnRecord>({
    ...record,
  ...
  const setField = (k: keyof SalesReturnRecord, v: any) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  return (
    ...
    >
      <SheetContent
        ...
        data-ocid="sales_return_history.edit.sheet"
      >
        <SheetHeader>
          <SheetTitle>Edit Sales Return — {record.returnNo}</SheetTitle>
        </SheetHeader>
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-3">
            {(
              [
                ...
                ["remarks", "Remarks"],
              ] as [keyof SalesReturnRecord, string, string?][]
            ).map(([k, label, type]) => (
              <div key={k}>
          ...
          <div className="border rounded overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  {["Medicine", "Batch", "Expiry", "Qty", "MRP", ""].map(
                    (h) => (
        ...
        <SheetFooter className="gap-2">
          <Button
            variant="outline"
            onClick={onClose}
            data-ocid="sales_return_history.edit_cancel.button"
          >
            Cancel
          </Button>
          <Button
            onClick={handleSave}
            disabled={saving}
            data-ocid="sales_return_history.edit_save.button"
          >
...
export default function SalesReturnHistory() {
  ...
  const [viewRecord, setViewRecord] = useState<SalesReturnRecord | null>(null);
  const [editRecord, setEditRecord] = useState<SalesReturnRecord | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<SalesReturnRecord | null>(
    null,
  );
  ...
  useEffect(() => {
    setRecords(
      [...getSalesReturns()].sort((a, b) => b.createdAt - a.createdAt),
    );
  }, []);
  ...
  const handleEdit = (updated: SalesReturnRecord) => {
    updateSalesReturn(updated);
    reload();
    setEditRecord(null);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      // Reverse stock: sales return increased stock, so delete decreases it
      for (const item of deleteTarget.items) {
        const med = medicines.find((m) => String(m.id) === item.medicineId);
        if (med) {
          const newStock = med.currentStock - BigInt(item.qty);
      ...
      deleteSalesReturn(deleteTarget.id);
      reload();
  ...
  const handlePrint = (r: SalesReturnRecord) =>
    openReturnPrintWindow(r, "sales");
  const handleDownload = (r: SalesReturnRecord) => {
    const html = buildReturnInvoiceHtml(r, "sales");
  ...
  return (
    <div className="space-y-5" data-ocid="sales_return_history.page">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-lg bg-amber-100">
          <History className="h-5 w-5 text-amber-600" />
        </div>
        <div>
          <h1 className="text-2xl font-bold">Sales Return History</h1>
          <p className="text-muted-foreground text-xs mt-0.5">
            View and manage all recorded sales returns
          </p>
        </div>
      </div>
        ...
        <CardHeader className="pb-3 pt-4 px-5">
          <div className="flex items-center justify-between">
            ...
            <div className="relative w-56">
              ...
              <Input
                data-ocid="sales_return_history.search.input"
              ...
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell
                    ...
                    data-ocid="sales_return_history.list.empty_state"
                  >
                ...
                filtered.map((r, idx) => (
                  <TableRow
                    ...
                    data-ocid={`sales_return_history.list.item.${idx + 1}`}
                  >
                    ...
                    <TableCell className="pr-5">
                      <div className="flex items-center gap-1 justify-end">
                        <Button
                          ...
                          title="View"
                          data-ocid={`sales_return_history.view_button.${idx + 1}`}
                        >
                        ...
                        <Button
                          ...
                          title="Edit"
                          data-ocid={`sales_return_history.edit_button.${idx + 1}`}
                        >
                        ...
                        <Button
                          ...
                          title="Print"
                          data-ocid={`sales_return_history.print_button.${idx + 1}`}
                        >
                        ...
                        <Button
                          ...
                          title="Download"
                          data-ocid={`sales_return_history.download_button.${idx + 1}`}
                        >
                        ...
                        <Button
                          ...
                          title="Delete"
                          data-ocid={`sales_return_history.delete_button.${idx + 1}`}
                        >
      ...
      >
        <DialogContent
          ...
          data-ocid="sales_return_history.view.dialog"
        >
          <DialogHeader>
            <DialogTitle>Sales Return — {viewRecord?.returnNo}</DialogTitle>
          </DialogHeader>
          {viewRecord && (
            <ScrollArea className="max-h-[60vh]">
              <div className="space-y-3 text-sm">
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div>
                    <span className="text-muted-foreground">Dr Reg No:</span>{" "}
                    {viewRecord.doctorRegNo}
                  </div>
                  {viewRecord.remarks && (
      ...
      >
        <AlertDialogContent data-ocid="sales_return_history.delete.dialog">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Sales Return?</AlertDialogTitle>
            <AlertDialogDescription>
              This will delete <strong>{deleteTarget?.returnNo}</strong> and
              reverse all stock changes (stock will decrease). This cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              disabled={deleting}
              data-ocid="sales_return_history.delete_cancel.button"
            >
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              ...
              disabled={deleting}
              data-ocid="sales_return_history.delete_confirm.button"
            >
